...
enum {
	...
	TP_DOOROTHERPARA=0x8070,		//新增客户定制需求协议，实现管理中心对门口机的安防、撤防、开锁等功能的操作
	TP_REQUESTVIDEO_OPEN = 0x20000,		//请求视频数据协议
	TP_REQUESTVIDEO_CLOSE = 0x20001,	//停止视频数据请求
...
typedef struct
{
	unsigned char IP[16];			//本机IP		
	unsigned int Port;				//本机端口
	unsigned int Cmd;				//回复结果Command
...
typedef struct{
	...
	unsigned int Size;					//包大小
	unsigned int Type;					//包类型
	unsigned long long int ReqDevID;		//设备ID
...
typedef struct{
	...
	unsigned	int Size;					//包大小
	unsigned	int Type;					//包类型
	unsigned	int AckFlag;				//应答标志	0发起	1应答
	unsigned	int ReadWrite;				//读写标志	0读出	1写入
	unsigned	int UseCardDisDef;			//刷卡撤防标志	1生效	0失效
	unsigned	int DisUnlockWhenMon;		//禁止开锁标志	1生效	0失效
	unsigned 	int Reserve[64];  			//保留
}TSetDefenseAndLock;
